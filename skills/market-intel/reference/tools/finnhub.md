...
- **Onboarding gotcha — `FINNHUB_STORAGE_DIR` must exist before first run.** The MCP needs a writable cache dir; create it before applying (e.g. `mkdir -p the local finnhub data store`) or the stdio process exits immediately on launch with an opaque error.
