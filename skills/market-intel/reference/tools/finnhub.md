...
- **Onboarding gotcha — `FINNHUB_STORAGE_DIR` must exist before first run.** The MCP needs a writable cache dir; create it before applying (e.g. `mkdir -p ~/.claude/data/finnhub-storage`) or the stdio process exits immediately on launch with an opaque error.
