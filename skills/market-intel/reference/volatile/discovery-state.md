...
| Steph-ux/x-mcp-playwright · alarok/x-agent-sdk (+4 siblings) | x-twitter | 0 to 35★ | adoption ~0, weeks-old single-author repos, several with no license; one (proxidize/x-scraper) is vendor-authored by a proxy seller | 2026-09 |
...
**2026-09-09 second light pulse (independent re-run; H1 only). Corrects the 2026-09-06 headline above.**

The 2026-09-06 pulse states that no H1 class produced "a barrier route opening or closing, or an
acquisition affecting commercial data." An independent re-scan finds **three** such events inside that
same window, all confirmed at a source that returns HTTP 200 today. The verdict this pulse reaches is
still *no new territory*, every item is FOLD, no NEW-DOMAIN, no NEW-SKILL, but the earlier headline
was wrong about the inputs, and its own watchlist row for `zedeus/nitter` already contradicted it.
Recording the correction rather than overwriting it, so the disagreement stays visible.

| event | class | verdict | disposition |
|---|---|---|---|
| **X Corp C&D against Nitter + XCancel**, `zedeus/nitter` issue #1442 opened 2026-08-25 (33 comments, still open); all public instances down by 2026-08-27; repo **not** archived, pushed 2026-09-07, project says it will continue | H1 barrier route closing | FOLD → x-twitter | **LANDED this run** as a legal-overhang line on the existing Nitter tombstone. Explicitly **not** a D-404: the repo is machine-alive, so C4 forbids tombstoning it. The change is to the *self-host* advice, the risk is now named-operator legal exposure, not just session tokens |
| **Nvidia to acquire Hugging Face**, official announcement at `blogs.nvidia.com/blog/nvidia-to-acquire-hugging-face/` (200 OK 2026-09-09) | H1 acquisition | FOLD → frontier-research | Risk note only; no API, price or route has changed yet. Touches two live assets: the HF Daily Papers row in `domains/frontier-research.md` and **discovery surface E3** (`huggingface.co/api/spaces?sort=trendingScore`) in `refresh-protocol.md`. ⚠ The widely-quoted ~$12.9B figure comes from third-party blogs, **not** the official page, per C5/C6 do not quote a number until an official filing is fetched. No shard edit this run (frontier-research is out of scope for a volatile sweep) |
| **AWS acquires DuckLabs**, announced 2026-08-26 at `duckdb.org/2026/08/26/ducklabs-to-join-aws` + the AWS Big Data blog; DuckDB/DuckLake stay open source under the DuckDB Foundation | H1 acquisition | FOLD → none | Logged so the next scan does not re-discover it as news. DuckDB is local analytics infrastructure, not a queryable commercial data source; the matrix cites it nowhere. **No shard edit, no watchlist row.** |
| **Cloudflare Kitesurf** (`blog.cloudflare.com/kitesurf/`, 2026-08-07, 221 HN pts): from-scratch non-Chromium browser engine in V8 isolates on Workers, CDP-compatible | H1 candidate new category | FOLD → browser-automation | Closest thing the window had to a new *category*, and it still is not one: a cheaper substrate under the same route-④ tooling. **Cloudflare's own post says it cannot negotiate TLS-fingerprint bot challenges**, so it does not touch the anti-detect picks (camoufox/nodriver/patchright). Beta, no self-host. WATCH, no shard edit |

**Landed this run:** `jackwener/OpenCLI` → browser-automation (29,133★ / 2,855 forks / 62 watchers /
248 open issues, Apache-2.0, created 2026-03-14, pushed 2026-08-30, gh-api 2026-09-09). 182 per-site
adapters over a real logged-in Chrome profile. Cleared adversarial verification on the strength of
**npm, not stars**: 84,338 downloads last month at a flat ~3k/day weekday profile
(`api.npmjs.org/downloads/range/last-month/@jackwener/opencli`). Its 470:1 star:watcher ratio is the
one weak spot and the reason to re-check the star slope next sweep; the long-tail contributor curve
(933/85/65/30/27/21/… across 100+ accounts) and 2,855 forks are what argue against inflation.
Its HN submission drew **6 points**, so there is no community troubleshooting base yet.

| candidate | domain | stars (gh-api 2026-09-09) | route | why watch (not yet ADD) | first_seen |
|---|---|---|---|---|---|
| 0xchasercat/draco | web-scraping | 97★ (5 forks, 2 open issues, 1 contributor; created 2026-07-05, pushed 2026-09-05) | ③ self-host, built-in MCP | **from the weekly E5 inbox** (Show HN, 11 pts / 6 comments, under the E5 ≥30-point bar). Single-binary Rust "Firecrawl drop-in you own entirely." Fails the adoption gate on every axis: one contributor, no independent third-party report. Notable mainly as the **second** instance of the drop-in-Firecrawl-replacement shape after `us/crw`, if a third appears, that pattern is worth a line in the web-scraping shard even if no single implementation clears the bar | 2026-09 |

**Re-run cross-validation (worth recording as a property of the process, not of any tool).** This pulse
was produced by an independent 13-agent discovery + adversarial-verify run that did not read the
2026-09-06 results first. On the five volatile domains it **independently reproduced every 2026-09-06
verdict it overlapped**: `dreammis/social-auto-upload` ADD, and HOLD/WATCH on `Tencent/BrowserSkill`,
`PawiX25/twifork`, `Altimis/Scweet`, `nirholas/XActions`, `CloakHQ/CloakBrowser`. Two agreeing runs are
weak evidence individually, same model, same cutoff, per the D5b correlation lesson, but the
disagreements are the informative part, and there were exactly two: the missed H1 events above, and
`jackwener/OpenCLI`, which the 2026-09-06 sweep did not surface at all.

**Structural changes proposed by this run: none.** The 2026-07 prediction-markets NEW-DOMAIN proposal
remains pending human approval and this run does not land it.
