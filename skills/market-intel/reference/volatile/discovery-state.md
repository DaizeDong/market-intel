...
| **Public Telegram channels as alt-data** (crypto alpha groups, news-break channels, regional discourse; candidate readers Telethon / telegram-mcp) | 2026-06 (1st) | FOLD → x-twitter / crypto-defi / trends-discovery | Genuinely uncovered as a *data source* (Telegram is in-matrix only as a notification channel in `discount-bandit.md`). Surfaced via the Horizon (Thysrael/Horizon) source list, **not** independently verified → fails H2 (needs ≥3 API-verified readers + adoption) + recurrence (1st sighting), and OpenBB on the same list is an aggregator over in-matrix providers (D4 套壳). Do NOT hand-add a primitive; next Discovery phase finds concrete readers and routes FOLD-vs-NEW-DOMAIN via human PR (C9). |
