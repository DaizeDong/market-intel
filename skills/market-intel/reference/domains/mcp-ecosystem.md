...
5. **No domain match → horizon-scan path**: if a candidate represents a new **territory** none of the 15 shards covers (e.g. a new agent-memory MCP category, a new modality), it is NOT promoted to an existing shard. Instead it goes into `horizon-scan` H2 with FOLD / NEW-DOMAIN / NEW-SKILL verdict — see refresh-protocol H1–H4.
