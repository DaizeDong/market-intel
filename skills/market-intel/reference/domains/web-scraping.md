...
| **wigolo** (5.1k★) | ③/④ free | local-first **search** + fetch + crawl + extract in one MCP, no API key: 18-engine SERP fusion + SearXNG fallback, HTTP→headless auto-escalation | `npx wigolo init --agents=claude-code` (github.com/KnockOutEZ/wigolo) | fills the shard's **only free-search gap** (Tavily/Exa are both ②-paid and Tavily's free tier caps at 1k credits/mo). ⚠ public beta, npm pinned behind the repo, ~98% single-maintainer; **does not bypass Cloudflare/DataDome** — returns a labelled `blocked_by_challenge` and wants your own proxy for IP-reputation walls |
