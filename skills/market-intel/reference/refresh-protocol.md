...
> 调度脚本 `~/.claude/scripts/refresh-market-intel.sh` 已实现 1/5/6 的编排骨架与 scope guard（拒绝越界改动）。
...
- Scheduled: Windows Task `RefreshMarketIntel` (monthly) runs `~/.claude/scripts/refresh-market-intel.sh`.
