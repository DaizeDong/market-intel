...
| **L3 ops state (recommended)** | `reference/companion-config-repo.md` (overview + tutorial) + `reference/companion-config-spec.md` (formal contract, version 1) | the recommended pattern for managing **your** install state — which tools *you* installed, *your* tier, *your* key rotation history — in a **per-user private companion repo** separate from this public matrix. Each user maintains their own; there is no canonical shared one. Templates committed, secrets gitignored, backed up out-of-band. **Mechanically interfaceable**: the formal spec defines a stable contract so any conforming companion repo can be consumed by this skill (or future tooling). |
